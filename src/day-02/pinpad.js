...
export class DiamondPinPad {
  static applyInstruction (position, instruction) {
    const [x, y] = position
    switch (instruction) {
      case 'U': return [x, Math.max(y - 1, Math.abs(x) - 2)]
      case 'D': return [x, Math.min(y + 1, 2 - Math.abs(x))]
      case 'L': return [Math.max(x - 1, Math.abs(y) - 2), y]
      case 'R': return [Math.min(x + 1, 2 - Math.abs(y)), y]
      default: throw Error(`Invalid instruction character: ${instruction}`)
    }
  }
